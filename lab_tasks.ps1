...
$Password2 = ConvertTo-SecureString "FrostyTheAdmin123!" -AsPlainText -Force

if (-not (Get-LocalUser -Name "jack.frost.admin" -ErrorAction SilentlyContinue)) {
    $User = "jack.frost.admin"
    New-LocalUser -Name $User -Password $Password2 -FullName "Jack Frost Admin" -Description "Pass: FrostyTheAdmin123!"
...
New-StoredCredential -Target "globomantics/wks01" -Username "jack.frost.admin" -Password "FrostyTheAdmin123!" -Persist LocalMachine
...
if(Test-Path "C:\Services\Bin Files\AdminTools.exe") {
    ...
    $pass = ConvertTo-SecureString "FrostyTheAdmin123!" -AsPlainText -Force
